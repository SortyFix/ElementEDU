...
public class UserServiceMockitoTest extends ServiceMockitoTest<UserService, UserEntity, UserModel, UserCreateModel>
{
    ...
    {
        UserCreateModel userCreateModel = new UserCreateModel("test", "test", "test", "Password123!", true, false, UserStatus.PROSPECTIVE);
    ...
    @Override protected @NotNull ServiceMockitoTest.TestExpectation<UserCreateModel, NameOccupiedException> occupiedData()
    {
        UserCreateModel request = new UserCreateModel("test", "test", "max.mustermann", "Password123!", true, false, UserStatus.PROSPECTIVE);
        NameOccupiedException expected = new NameOccupiedException("max.mustermann");
        return TestExpectation.data(request, expected);
    }

    @Test void testCreateUserPasswordInsecureMockito()
    {
        UserCreateModel request = new UserCreateModel("test", "test", "max.mustermann", "password", true, false, UserStatus.PROSPECTIVE);
