...
export abstract class EntityService<P, T extends { id: P }, G, C> {
    ...
    protected constructor(private readonly _http: HttpClient, private readonly _location: string,) {}
    ...
    public create(models: C[]): Observable<T[]> {
        ...
        return this.http.post<G[]>(url, this.packets(models), {withCredentials: true}).pipe(this.translateValue, tap((response: T[]): void => this.pushCreated(response)));
    }
    ...
    protected packets(models: C[]): any[] {
